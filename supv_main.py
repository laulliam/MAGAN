...
def main():
    ...
    recorder = Recorder(args.snapshot_pref, ignore_folders=["Exps", 'data', 'visual_att'])
...
def train_epoch(model, train_dataloader, criterion, criterion_event, optimizer, epoch):
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        is_event_scores, event_scores, um_gate, um_scores, mm_gate, mm_scores = model(visual_feature, audio_feature)
        is_event_scores = is_event_scores.transpose(1, 0).squeeze().contiguous()
        um_gate = um_gate.transpose(1, 0).squeeze().contiguous()
        mm_gate = mm_gate.transpose(1, 0).squeeze().contiguous()

        loss_is_event = criterion(is_event_scores, labels_BCE.cuda())
        loss_event_class = criterion_event(event_scores, labels_event.cuda())
        loss_um_gate = criterion(um_gate, labels_BCE.cuda())
        loss_um_score = criterion_event(um_scores, labels_event.cuda())
        loss_mm_gate = criterion(mm_gate, labels_BCE.cuda())
        loss_mm_score = criterion_event(mm_scores, labels_event.cuda())
        loss_gate = 0.5 * loss_um_gate + 0.5 * loss_mm_gate
        loss_scores = 0.5 * loss_um_score + 0.5 * loss_mm_score

        loss = loss_is_event + loss_event_class + loss_gate + loss_scores
...
def validate_epoch(model, test_dataloader, criterion, criterion_event, epoch, eval_only=False):
    ...
    for n_iter, batch_data in enumerate(test_dataloader):
        ...
        acc = compute_accuracy_supervised(is_event_scores, event_scores, labels)
        accuracy.update(acc.item(), batch_size * 10)

        batch_time.update(time.time() - end_time)
        end_time = time.time()
        losses.update(loss.item(), batch_size * 10)
...
def compute_accuracy_supervised(is_event_scores, event_scores, labels):
    ...
    scores_pos_ind = is_event_scores > 0.7
...
def save_checkpoint(state_dict, top1, task, epoch):
    if not os.path.exists(f'{args.snapshot_pref}/checkpoints/'):
        os.mkdir(f'{args.snapshot_pref}/checkpoints/')
    model_name = f'model_epoch_{epoch}_top1_{top1:.3f}_task_{task}_best_model.pth.tar'
    torch.save(state_dict, f'{args.snapshot_pref}/checkpoints/{model_name}')


if __name__ == '__main__':
    main()
    # mainModel = main_model(config['model'])
    # print(summary(mainModel, torch.zeros((1, 10, 7, 7, 512)), torch.zeros((1, 10, 128)), show_input=True))
