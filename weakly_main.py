...
def train_epoch(model, train_dataloader, criterion, criterion_event, optimizer, epoch):
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        labels_event = labels.cuda()

        is_event_scores, event_scores, gates, gates_score = model(visual_feature, audio_feature)

        # loss_is_event = criterion(is_event_scores, labels_BCE.cuda())
        loss_event = criterion_event(event_scores, labels_event.cuda())
        loss_gates = criterion_event(gates_score, labels_event.cuda())
        loss = loss_event + loss_gates
        loss.backward()
...
def validate_epoch(model, test_dataloader, criterion, criterion_event, epoch, eval_only=False):
    ...
    for n_iter, batch_data in enumerate(test_dataloader):
        ...
        is_event_scores, event_scores, gates, gates_scores = model(visual_feature, audio_feature)
        event_scores = event_scores + gates_scores
        acc = compute_accuracy_supervised(is_event_scores, event_scores, labels_event)
