...
class Recorder(object):
    def __init__(self, snapshot_pref, ignore_folders):
        # if not os.path.isdir(snapshot_pref):
        #     os.mkdir(snapshot_pref)
        if not os.path.exists(snapshot_pref):
            os.mkdir(snapshot_pref)
        else:
            FileExistsError
        ...
        self.copy_code(dst=self.code_path, ignore_folders=ignore_folders)
    ...
    def copy_code(self, src="./", dst="./code/", ignore_folders=['Exps']):
        if os.path.isdir(dst):
            # dst = "/".join(dst.split("/")[:-1])+"_"+str(uuid.uuid4())+"/"
            dst = "/".join(dst.split("/")) + "code/"
        file_abs_list = []
        src_abs = os.path.abspath(src)
        for root, dirs, files in os.walk(src_abs):
            if all(key not in root for key in ignore_folders):
                for name in files:
